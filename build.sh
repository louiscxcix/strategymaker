#!/bin/bash
# Docker Buildx Build Script for strategymaker
...
Build Docker image for $strategymaker using buildx.
...
echo -e "${BLUE}📦 Building $strategymaker...${NC}"
...
BUILD_CMD="$BUILD_CMD --tag=$REGISTRY/$strategymaker:latest"
BUILD_CMD="$BUILD_CMD --tag=$REGISTRY/$strategymaker:$TIMESTAMP"
...
if eval "$BUILD_CMD"; then
    echo ""
    echo -e "${GREEN}✅ Successfully built $strategymaker${NC}"
    echo -e "  Image: $REGISTRY/$strategymaker:latest"
    echo -e "  Image: $REGISTRY/$strategymaker:$TIMESTAMP"
else
    echo ""
    echo -e "${RED}❌ Failed to build $strategymaker${NC}"
    exit 1
fi
